...
public class SlideHolder extends FrameLayout {
	...
	private boolean mMenuMove = true;
	...
	protected void dispatchDraw(Canvas canvas) {
		try {
			if(mMode == MODE_SLIDE) {
				...
				// TODO: determine what is does - possible this is no effect ?
				int incrementX = mMenuMove && (mOffset-menuWidth<0) ? mOffset-menuWidth : 0;
				canvas.translate(-scrollX + incrementX, -scrollY);
				menu.draw(canvas);
				canvas.restore();
	...
	private boolean handleTouchEvent(MotionEvent ev) {
		...
		if(ev.getAction() == MotionEvent.ACTION_MOVE) {
			...
			if((mDirection*diff > mThreshold && mMode == MODE_READY) || (mDirection*diff < -mThreshold && mMode == MODE_FINISHED)) {
				mHistoricalX = (int) x;
